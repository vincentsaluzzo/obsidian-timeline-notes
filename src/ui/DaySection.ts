import { MarkdownView, Notice, TFile, WorkspaceLeaf, Component } from 'obsidian';
import { DailyNoteManager } from '../daily-notes/DailyNoteManager';
import { DateUtils } from '../utils/DateUtils';
import TimelineNotesPlugin from '../../main';
import { spawnLeafInContainer, openFileInLeaf, detachEmbeddedLeaf } from '../utils/EmbeddedLeaf';
...
export class DaySection extends Component {
    ...
    constructor(container: HTMLElement, date: Date, plugin: TimelineNotesPlugin, dailyNoteManager: DailyNoteManager) {
        super();
    ...
    async render(): Promise<void> {
        ...
        if (!this.file) {
            new Notice(`Failed to load file for ${DateUtils.formatDateString(this.date)}`);
            return;
        }

        // Create editor area
        this.editorContainer = this.container.createDiv({ cls: 'timeline-day-editor' });
    ...
    private async createEmbeddedEditor(): Promise<void> {
        if (!this.file || !this.editorContainer) {
            console.error('Missing file or container', { file: this.file, container: this.editorContainer });
            return;
        }

        try {
            // Spawn an embedded leaf in the container
            this.embeddedLeaf = spawnLeafInContainer(this.plugin, this.editorContainer);
            ...
            await openFileInLeaf(this.embeddedLeaf, this.file, this.editorContainer, 'source');

        } catch (error) {
            console.error('Error creating embedded editor for', this.date.toDateString(), error);
    ...
    private formatDateHeader(date: Date): string {
        ...
        const dateStr = this.dailyNoteManager.getFilenameForDate(date).replace('.md', '');
        return dateStr;
    }
