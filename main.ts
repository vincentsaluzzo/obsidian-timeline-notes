...
let CalendarView:
  | typeof import("./src/ui/CalendarView").CalendarView
  | undefined;
let CALENDAR_VIEW_TYPE: string;
let AuthManager:
  | typeof import("./src/calendar/AuthManager").AuthManager
  | undefined;
let GoogleCalendarAPI:
  | typeof import("./src/calendar/GoogleCalendarAPI").GoogleCalendarAPI
  | undefined;
...
export default class TimelineNotesPlugin extends Plugin {
  ...
  calendarAPI: InstanceType<NonNullable<typeof GoogleCalendarAPI>> | null =
    null;
  ...
  async onload() {
    ...
    if (!isMobile) {
      try {
        ...
        console.error(
          "Failed to load Google Calendar modules (desktop features will be unavailable):",
          error
        );
      }
    }
